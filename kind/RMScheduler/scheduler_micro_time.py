...
def evaluate(lp, pods):
    ...
    for i, pod in enumerate(pods):
        if i not in lp.scheduler_tasks["sol"]:
...
if __name__ == "__main__":
    ...
    # num_servers = 10
    with open("RMScheduler/scheduler_micro_time.csv", "w") as out:
        out.write(
            "Pod_to_node_resource_ratio, prob, nodes_del_%, lp_time, heuristic_time, heuristic_mig_time, lpmig_time, seed, num_servers, skewed_capacity\n"
        )
    out.close()
    print("Running configs now...")
    for num_servers in [10, 20]:
        for seed in [6, 7, 8, 9, 10]:
            np.random.seed(seed)
            for num_servers in [20]:
                avg_server_rsc = 1000
                    ...
                    for pod_to_node_resource_ratio in [
                        ...
                        for prob in [0.9]:
                            ...
                            for nodes_to_del_percent in [1, 2, 3, 4, 5, 6, 7, 8, 9]:
                                nodes_to_del = int(
                                    num_servers * nodes_to_del_percent * 0.1
                                )
                                ...
                                nextfit = HeuristicScheduler(cluster_state)
                                nextfit.make_schedule()
                                ...
                                nextfit_wmig = HeuristicScheduler(cluster_state)
                                nextfit_wmig.make_schedule()

                                # res_heu = evaluate(nextfit, planner_o)
                                # res_heu_mig = evaluate(nextfit_wmig, planner_o)
                                ...
                                lpmig = LPScheduler(cluster_state)
                                lpmig.make_schedule()

                                # res_lp = evaluate(lp, planner_o)

                                # res_lpmig = evaluate(lpmig, planner_o)

                                with open(
                                    "RMScheduler/scheduler_micro_time.csv", "a"
                                ) as out:
                                    out.write(
                                        str(pod_to_node_resource_ratio)
                                        + ","
                                        + str(prob)
                                        + ","
                                        + str(nodes_to_del_percent)
                                        + ","
                                        + str(lp.time_breakdown["scheduling_time"])
                                        + ","
                                        + str(nextfit.time_breakdown["scheduling_time"])
                                        + ","
                                        + str(
                                            nextfit_wmig.time_breakdown[
                                                "scheduling_time"
                                            ]
                                        )
                                        + ","
                                        + str(lpmig.time_breakdown["scheduling_time"])
                                        + ","
                                        + str(seed)
                                        + ","
                                        + str(num_servers)
